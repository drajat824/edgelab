export default function ScriptReference() {
  ...
  const functions = [
    {
      ...
      description:
        "Increase or decrease CPU frequency by x steps.",
    },
    {
      name: "hold_frequency()",
  ...
  return (
    <div className="w-fit flex flex-col gap-6">
      ...
      {/* Header */}
      <div>
        <h2 className="text-lg font-bold text-[var(--menu)]">
          Available Variables & Functions
        </h2>

        <p className="text-sm text-gray-500 mt-1">
          These variables and helper functions can be used
          inside the userspace script editor.
        </p>
      </div>
          ...
          {variables.map((item) => (
            ...
            >
              <div className="flex justify-between items-start flex-wrap gap-2">
                ...
                <div>
                  <code className="font-semibold text-[var(--menu)]">
                    {item.name}
                  </code>

                  <span className="ml-2 px-2 py-1 rounded-md text-xs bg-blue-50 text-blue-600">
                    {item.type}
                  </span>
                </div>
          ...
          {functions.map((item) => (
            ...
            >
              <div className="flex justify-between items-start flex-wrap gap-2">
                ...
                <code className="font-semibold text-green-600">
                  {item.name}
                </code>
