...
export default function Droppable({ id, children, onMouseDown, isEmpty = true, disabled }) {
  ...
  return (
    disabled ? (
      ...
      <div
        className="relative w-13 h-20 border border-gray-200 bg-gray-400 rounded flex items-center justify-center cursor-not-allowed opacity-60">
        {children}
      </div>
    ) : (
      ...
      <button
        ...
        ref={ref}
        className={`relative w-13 h-20 border border-dashed rounded flex items-center justify-center transition-all duration-300 ${!isEmpty ? "cursor-pointer hover:shadow-md hover:-translate-y-0.5" : "border-gray-500"}`}
