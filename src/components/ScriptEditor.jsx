...
export default function ScriptEditor({ title = "Python Script Editor", value = "", onChange, onSave, disabled, placeholder = `# Example\nif cpu_temp > 70:\n    set_min_frequency()\n`, errorMessage = "", isDirty = false }) {
  ...
  const handleSave = () => {
    ...
    if (validationError) {
      ...
      if (onSave) {
        onSave(formattedCode, validationError);
      }
      return;
    }
    if (onSave) {
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col w-full">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b bg-gray-50">
          <div>
            ...
            <p className="text-sm text-gray-500">Write your userspace governor logic.</p>
          </div>
        ...
        <div className="flex px-5 py-4 border-t bg-gray-50">
          <button
            onClick={handleSave}
            disabled={disabled || !value.trim() || !isDirty || errorMessage}
            className="
                        px-5 py-2
                        rounded-xl
                        bg-blue-600 
